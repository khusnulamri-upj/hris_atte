...
class User_model extends CI_Model {
	...
	function login()
	{
		...
		{	
			...
			$this->data['message'] = validation_errors('<p class="message dismissible error">', '</p>');
	...
	function resend_activation_token()
	{
		...
		{					
			...
			// Redirect user.
			($response) ? redirect('user') : redirect('user/resend_activation_token');
		}
		else
		{	
			// Set validation errors.
	...
	function update_account()
	{
		...
		if ($this->form_validation->run())
		{
			...
			// Redirect user.
			($response) ? redirect('members/dashboard') : redirect('members/update_account');
		}
		else
		{		
			// Set validation errors.
	...
	function change_password()
	{
		...
		if ($this->form_validation->run())
		{
			...
			($response) ? redirect('members/dashboard') : redirect('members/change_password');
		}
		else
		{		
			// Set validation errors.

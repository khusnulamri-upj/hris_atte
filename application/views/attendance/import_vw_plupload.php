...
?>
    ...
    <div class="container">
      ...
      <div class="row padded" id="for-plupload">
        <div class="one half">
      ...
      <div class="row padded" id="for-plupload2">
      ...
      <div class="row padded">
        <div class="one whole" id="existingfilemanual"></div>
      </div>
      <div class="row padded">
          <div class="two tenth">
            <a role="button" id="processfilesmanual" href="#">Process File</a>
          </div>
      </div>
...
.text( "Does nothing..." );
    ...
    //$('#processfilesmanual').click(function(){aj()});
    
    function checkFile() {
        $.ajax({
                type:'POST',
                url: '<?= base_url('import/mdb_existing_file') ?>',
                success:function(data){
                    var rspn = data.split('||');
                    $('#existingfilemanual').empty();
                    $('#existingfilemanual').append(rspn[1]);
                    if (rspn[0] === '1') {
                        $('#processfilesmanual').bind("click",aj);
                        $('#processfilesmanual').removeAttr("disabled");
                    } else {
                        //$('#processfilesmanual').unbind("click",aj);
                        $('#processfilesmanual').bind("click",jftp);
                        $('#processfilesmanual').attr("disabled","disabled"); 
                    }
                },
                error: function(xhr, status, error) {
                    var err = eval("(" + xhr.responseText + ")");
                    $('#existingfilemanual').empty();
                    $('#existingfilemanual').append(err.Message);
                }
            });
    };
    checkFile();
    function jftp() {
        $.ajax({
                type:'POST',
                url: '<?= base_url('import/run_jftp_attendance') ?>'
            });
    };
    </script>
<?php
